...
[DependsOn(
    ...
    typeof(VitalLinkEntityFrameworkCoreModule),
    typeof(VitalLinkHttpApiModule),
...
public class VitalLinkHttpApiHostModule : AbpModule
{
    ...
    public override void ConfigureServices(ServiceConfigurationContext context)
    {
        ...
        if (hostingEnvironment.IsDevelopment())
        {
            ...
            {
                options.FileSets.ReplaceEmbeddedByPhysical<VitalLinkDomainSharedModule>(Path.Combine(hostingEnvironment.ContentRootPath, string.Format("..{0}..{0}src{0}VitalLink.Domain.Shared", Path.DirectorySeparatorChar)));
                options.FileSets.ReplaceEmbeddedByPhysical<VitalLinkDomainModule>(Path.Combine(hostingEnvironment.ContentRootPath, string.Format("..{0}..{0}src{0}VitalLink.Domain", Path.DirectorySeparatorChar)));
                options.FileSets.ReplaceEmbeddedByPhysical<VitalLinkApplicationContractsModule>(Path.Combine(hostingEnvironment.ContentRootPath, string.Format("..{0}..{0}src{0}VitalLink.Application.Contracts", Path.DirectorySeparatorChar)));
                options.FileSets.ReplaceEmbeddedByPhysical<VitalLinkApplicationModule>(Path.Combine(hostingEnvironment.ContentRootPath, string.Format("..{0}..{0}src{0}VitalLink.Application", Path.DirectorySeparatorChar)));
            });
        }
        ...
        context.Services.AddAbpSwaggerGenWithOAuth(
            ...
            {
                { "VitalLink", "VitalLink API" }
            },
            options =>
            {
                options.SwaggerDoc("v1", new OpenApiInfo { Title = "VitalLink API", Version = "v1" });
